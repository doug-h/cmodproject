...
class PlanetSim(object):
    ...
    def read_params(self,fname):
        ...
        self.dt, self.sim_time, self.t0, = params
        self.time = self.t0
...
if __name__ == "__main__":
    ...
    S.run(traj_f)
    head = ["Planet","Apoapsis(m)","Periapsis(m)","T-counting(days)","T-fitting(days)"]
    table = [[p.label,p.apo,p.per,p.period,p.period2] for p in S.planet_list]
    table.insert(0,head)
    print_table(table)
    """print("dt:",S.dt,"sim.time:",S.sim_time,"t0:",S.t0)
